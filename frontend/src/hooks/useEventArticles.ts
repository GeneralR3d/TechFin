...
async function eventArticlesFetcher(url: string): Promise<GraphArticle[]> {
...
export function useEventArticles(eventId: string | null) {
  const { data, error, isLoading } = useSWR<GraphArticle[]>(
    eventId ? `/api/graph/events/${encodeURIComponent(eventId)}/articles?limit=30` : null,
    eventArticlesFetcher,
